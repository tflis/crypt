...
impl HasherConfig {
  ...
  pub fn new(algorithm: String, rounds: u32, salt: Vec<u8>) -> HasherConfig {
    HasherConfig { algorithm: algorithm, rounds: rounds, salt: salt }
  }
}
...
impl HasherData {
  ...
  pub fn new() -> HasherData {
    HasherData { configurations: HashMap::new(), latest_version: "0".to_string() }
  }
  ...
  pub fn get_all_versions(&self) -> Vec<&String> {
    self.configurations.keys().collect::<Vec<_>>()
  }
...
mod tests {
  ...
  fn bcrypt_algorithm() {
    ...
    let cfg = HasherConfig { rounds: rounds, salt: salt, algorithm: "bcrypt".to_string() };
  ...
  fn pbkdf2_algorithm() {
    let rounds = 10;
    let salt = [10u8, 16].to_vec();
    let cfg = HasherConfig { rounds: rounds, salt: salt, algorithm: "pbkdf2".to_string() };
  ...
  fn unknown_algorithm() {
    let rounds = 10;
    let salt = [10u8, 16].to_vec();
    let cfg = HasherConfig { rounds: rounds, salt: salt, algorithm: "unknown".to_string() };
  ...
  fn mixed_algorithm() {
    let rounds = 10;
    let salt = [10u8, 16].to_vec();
    let cfg1 = HasherConfig { rounds: rounds, salt: salt, algorithm: "bcrypt".to_string() };
    let salt = [10u8, 16].to_vec();
    let cfg2 = HasherConfig { rounds: rounds, salt: salt, algorithm: "pbkdf2".to_string() };
