...
impl Display for CryptError {
  fn fmt(&self, f: &mut ::std::fmt::Formatter) -> Result<(), ::std::fmt::Error> {
    match *self {
      ...
      CryptError::HasherNotFound(ref algorithm) => write!(f, "Hasher algorithm: {} not implemented", algorithm),
      CryptError::CipherNotFound(ref algorithm) => write!(f, "Cipher algorithm: {} not implemented", algorithm),
      CryptError::UnsupportedConfigVersion() => write!(f, "Crypt config version not supported"),
      CryptError::DuplicationCipherConfigVersion(ref version) => write!(f, "Duplication of cipher config version: {}", version),
      CryptError::DuplicationHasherConfigVersion(ref version) => write!(f, "Duplication of hasher config version: {}", version),
      CryptError::UnsupportedCipherAlgorithm(ref algorithm) => write!(f, "Unsupported cipher algorithm: {}", algorithm),
      CryptError::UnsupportedHasherAlgorithm(ref algorithm) => write!(f, "Unsupported hasher algorithm: {}", algorithm),
      ...
      CryptError::InvalidQuery(ref reason) => write!(f, "Invalid query: {}", reason),
      CryptError::OperationNotImplemented(ref info) => write!(f, "Operation not implemented: {}", info)
    }
  }
}
...
impl error::Error for CryptError {
  fn description(&self) -> &str {
    match *self {
      ...
      CryptError::DuplicationCipherConfigVersion(ref _version) => "Duplication of cipher config version",
      CryptError::DuplicationHasherConfigVersion(ref _version) => "Duplication of hasher config version",
      CryptError::UnsupportedCipherAlgorithm(ref _algorithm) => "Unsupported cipher algorithm",
      CryptError::UnsupportedHasherAlgorithm(ref _algorithm) => "Unsupported hasher algorithm",
      CryptError::BadIterationCount() => "Bad iteration count",
      CryptError::InvalidDocument(ref _reason) => "Invalid document",
      CryptError::InvalidQuery(ref _reason) => "Invalid query",
      CryptError::OperationNotImplemented(ref _info) => "Operation not implemented"
    }
  }
